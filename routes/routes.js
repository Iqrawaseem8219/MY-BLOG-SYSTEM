...
router.get('/blog', (req, res) => {
    // This route previously rendered the blog detail without data.
    // Redirect to home or a listing page instead to avoid empty view.
    res.redirect('/');
});
...
router.post('/posts', authenticateToken, upload.single('picture'), async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(authorId)) {
            throw new Error('Invalid author ID in token');
        }
...
router.post('/posts/update', authenticateToken, upload.single('picture'), async (req, res) => {
    try {
        ...
        post.tags = typeof tags === 'string' ? tags.split(',').map(tag => tag.trim()).filter(Boolean) : post.tags;
...
router.post('/posts/delete', authenticateToken, async (req, res) => {
    try {
        ...
        // Ensure the current user owns the post before deleting
        const post = await Post.findById(postId);
        if (!post) {
            return res.status(404).send('Post not found');
        }
